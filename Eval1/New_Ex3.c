...
int main()
{
	...
	/*	//Previous code for future reference
	do
	{
		i = j = 0;
		...
		if(j != 1) //If the node is not found, asking again
		{
			printf("\n Error: Data Not Found!");
			printf("\n Enter 1 to try again, 0 to continue... ");
			scanf(" %d", &i);
	...
	*/
	...
	//Outer loop for each data
	p = head;
	while(p != NULL)
	{
		pt = p;
		ptr = p->next;
		//Inner loop to check for duplicate instances of the data
		while(ptr != NULL)
		{
			//If duplicate is found delete the node
			if(ptr->data == p->data)
			{
				pt->next = ptr->next;
				free(ptr);
